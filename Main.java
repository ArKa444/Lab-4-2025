...
public class Main {
    ...
    private static void testAssignment9() throws IOException, ClassNotFoundException {
        ...
        TabulatedFunction listFunc = new LinkedListTabulatedFunction(
            new FunctionPoint[] {
                new FunctionPoint(0, 0), new FunctionPoint(1, 1), new FunctionPoint(2, 2),
                new FunctionPoint(3, 3), new FunctionPoint(4, 4), new FunctionPoint(5, 5),
                new FunctionPoint(6, 6), new FunctionPoint(7, 7), new FunctionPoint(8, 8),
                new FunctionPoint(9, 9), new FunctionPoint(10, 10)
            }
        );
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("list_serializable.ser"))) {
            out.writeObject(listFunc);
        }
        
        //десериализуем LinkedListTabulatedFunction
        TabulatedFunction listRestored;
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream("list_serializable.ser"))) {
